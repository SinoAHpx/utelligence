...
import { useDuplicatesStore } from "@/store";
import { Download, Filter, InfoIcon } from "lucide-react";

export default function DuplicatesVisualization() {
	// Use Zustand store directly
	const { data, selectedColumns, duplicateGroups, statistics, activeTab, setActiveTab } =
		useDuplicatesStore();
		...
		<Card className="w-full">
			<CardHeader>
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription>基于 {selectedColumns.length} 列检测到的重复数据</CardDescription>
					</div>
			...
			<CardContent className="space-y-4">
				<Tabs value={activeTab} onValueChange={setActiveTab}>
					...
					<TabsContent value="details">{renderDuplicateGroups()}</TabsContent>
				</Tabs>
			</CardContent>
		</Card>
	);
