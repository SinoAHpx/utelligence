...
import { CheckIcon, InfoIcon, Loader2, X } from "lucide-react";
import { useEffect, useState } from "react";
import type { DuplicatesTabProps } from "./types";
...
export default function DuplicatesTab({
	...
	// 获取store数据和函数
	const { processedData: storeRawFileData, cleanedData, cleanData } = useUnifiedDataStore();
	...
	const analyzeDuplicates = async () => {
		// 确保rawData存在并有效
		if (!effectiveRawData || !effectiveRawData.rows || !effectiveRawData.headers) {
			toast({
				title: "错误",
		...
		try {
			// 准备要发送的数据
			const dataForAnalysis = effectiveRawData.rows.map((row, idx) => {
				const rowData: Record<string, any> = {};
				effectiveRawData.headers.forEach((header, i) => {
	...
	const handleProcessDuplicates = async () => {
		if (!file || !effectiveRawData || columnsSelection.length === 0) {
			toast({
				title: "错误",
		...
		try {
			...
			onProgress(50); // 设置为50%，因为这通常是单个操作

			// 使用store的cleanData方法
			await cleanData("duplicates", {
				data: currentData,
				columnsToCheck: columnsSelection,
				strategy: keepStrategy,
				indices: [] // This would need to be calculated if needed
			});
	...
	if (!effectiveRawData) {
		return (
			<div className="space-y-4">
				<h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">重复数据处理选项</h4>
		...
		<div className="space-y-6">
					...
					{columns.map((column) => {
						...
						return (
							<Badge
								...
								onClick={() => {
									if (isSelected) {
										setColumnsSelection(columnsSelection.filter((c) => c !== column));
									} else {
			...
			{duplicateOption === "remove-duplicates" && (
				<div className="space-y-3 pl-6 mt-2">
					...
					<RadioGroup value={keepStrategy} onValueChange={setKeepStrategy} className="space-y-1">
						<div className="flex items-center space-x-2">
			...
			<div className="text-xs text-gray-500 bg-slate-50 dark:bg-slate-900 p-3 rounded-md border border-slate-200 dark:border-slate-800 mt-4">
				...
				<p>
					移除重复数据可以减小数据集大小并防止分析偏差，但应谨慎选择保留策略以确保保留最有价值的记录。
				</p>
			</div>
			...
			<div className="flex justify-between mt-4">
				...
				<Button
					...
					disabled={columnsSelection.length === 0}
				>
					{cleaningComplete ? (
						<>
						...
						<div className="bg-white dark:bg-gray-800 p-3 rounded-md border border-gray-200 dark:border-gray-700">
							...
							<p className="text-xl font-semibold">
								...
								<Badge variant="outline" className="ml-2 text-xs">
									{Math.round(
										(duplicateStats.duplicateCount / (effectiveRawData?.rows.length || 1)) * 100
									)}
									%
								</Badge>
							</p>
