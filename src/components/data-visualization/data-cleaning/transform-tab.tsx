...
import { CheckIcon, CircleIcon, Loader2, X } from "lucide-react";
import { useEffect, useState } from "react";
import type { TransformTabProps } from "./types";
	...
	if (!rawFileData) {
		return (
			<div className="space-y-4">
				<h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">数据转换选项</h4>
			...
			<div className="space-y-4">
				...
				<div>
					...
					<div className="flex flex-wrap gap-2 mb-3 max-h-[200px] overflow-y-auto p-3 border rounded-md">
						{availableColumns.map((column) => {
							const isSelected =
								selectedTransformColumns && selectedTransformColumns.includes(column);
							return (
								<Badge
									key={column}
				...
				<div className="text-xs text-gray-500 bg-slate-50 dark:bg-slate-900 p-3 rounded-md border border-slate-200 dark:border-slate-800">
					...
					<p className="mb-1">
						...
						{transformType === "categorical" &&
							"分类转换用于将分类数据转换为机器学习算法可用的数值形式。"}
					</p>
					<p>
						{transformType === "numeric" &&
							numericOperation === "normalize" &&
							"标准化将使数据均值为0，标准差为1，适用于正态分布的数据。"}
						{transformType === "numeric" &&
							numericOperation === "scale" &&
							"缩放转换将数据线性映射到指定的区间内。"}
						{transformType === "numeric" &&
							numericOperation === "log" &&
							"对数转换适用于处理偏态分布数据，可以降低异常值的影响。"}
						{transformType === "numeric" &&
							numericOperation === "square-root" &&
							"平方根转换适用于中等偏态分布数据，可以减小高值的权重。"}
						...
						{transformType === "text" &&
							textOperation === "trim" &&
							"去除首尾空格可以清理文本数据，避免因空格造成的不一致性。"}
						{transformType === "text" &&
							(textOperation === "prefix" || textOperation === "suffix") &&
							"添加前后缀可以用于标识处理过的数据或添加单位等。"}
						{transformType === "text" &&
							textOperation === "regex" &&
							"正则表达式替换可以用于复杂的文本模式匹配和替换，非常灵活。"}

						{transformType === "categorical" &&
							categoricalOperation === "one-hot" &&
							"独热编码将每个分类值转换为一个新的二元特征列，适用于名义变量。"}
						{transformType === "categorical" &&
							categoricalOperation === "label" &&
							"标签编码将分类值映射为整数，适用于序数变量。"}
					</p>
				</div>
				...
				<div className="flex justify-end pt-2">
					<Button
						...
						disabled={
							isTransforming || !selectedTransformColumns || selectedTransformColumns.length === 0
						}
					>
						{isTransforming ? (
							<>
