...
import { BarChart3Icon, CheckIcon, InfoIcon } from "lucide-react";
import { useEffect, useMemo, useState } from "react";
import type { OutliersTabProps } from "./types";
...
export default function OutliersTab({
	...
	useEffect(() => {
		...
		const settings: {
			[key: string]: { method: string; action: string; threshold?: number; multiplier?: number };
		} = {};
	...
	const handleProcessOutliers = async () => {
		...
		try {
			...
			for (const column of Object.keys(settings)) {
				...
				// Use the store's cleanData method
				await cleanData("outliers", {
					data: currentData,
					column,
					...columnSettings,
	...
	if (!effectiveRawData) {
		return (
			<div className="space-y-4">
				<h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">异常值检测与处理</h4>
	...
	return (
		<div className="space-y-6">
			...
			<div className="text-xs text-gray-500 bg-slate-100 dark:bg-slate-800 p-3 rounded-md border border-slate-200 dark:border-slate-700 mt-6">
				...
				<p className="mb-1">
					异常值是指显著偏离数据集主体分布的数据点，可能是由测量错误、数据输入错误或者真实的极端情况导致。
				</p>
				<p>
					Z-Score法基于标准差检测异常值，适用于近似正态分布的数据；IQR法基于四分位数间距，对分布无特殊要求，更稳健。
				</p>
			</div>
