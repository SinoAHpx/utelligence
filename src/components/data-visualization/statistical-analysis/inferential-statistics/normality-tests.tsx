	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>判断数据分布是否符合正态分布</CardDescription>
			</CardHeader>
			<CardContent className="space-y-6">
				<div>
					...
					<Alert variant={isNormal ? "default" : "destructive"}>
						<AlertDescription>
							根据 Jarque-Bera 检验结果，数据 {isNormal ? "符合" : "不符合"} 正态分布 (p
							{isNormal ? "≥" : "<"}0.05)
						</AlertDescription>
					</Alert>
				</div>
					...
					<Table>
						...
						<TableBody>
							<TableRow>
								...
								<TableCell>
									{jbResult.statistic !== null ? jbResult.statistic.toFixed(4) : "N/A"}
								</TableCell>
								<TableCell>
									{jbResult.pValue !== null ? jbResult.pValue.toFixed(4) : "N/A"}
								</TableCell>
								<TableCell>{isNormal ? "符合正态分布" : "不符合正态分布"}</TableCell>
							</TableRow>
						</TableBody>
					</Table>
				</div>
