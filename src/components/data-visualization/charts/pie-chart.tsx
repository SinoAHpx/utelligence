...
export const PieChartComponent: React.FC<PieChartComponentProps> = React.memo(({ chartConfig }) => {
	const { title = "Pie Chart", processedData = [], yAxisColumn, isTruncated = false } = chartConfig;
					...
					<PieChart>
						...
						>
							{pieData.map((entry, index) => (
								<Cell key={`pie-cell-${entry.name}-${index}`} fill={getChartColor(index)} />
							))}
						</Pie>
						<Tooltip />
						<Legend />
