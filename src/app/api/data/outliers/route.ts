...
export async function GET(request: Request) {
	...
	try {
		...
		return NextResponse.json({ error: "Error processing outlier data" }, { status: 500 });
	}
}
...
export async function POST(request: Request) {
	try {
		...
		return NextResponse.json({ error: "Error processing outliers" }, { status: 500 });
	}
}
...
function detectOutliers(
	...
	if (method === "zscore") {
		...
		const upperIndex = Math.min(
			...
			Math.floor(sortedValues.length * (upperPercentile / 100))
		);
